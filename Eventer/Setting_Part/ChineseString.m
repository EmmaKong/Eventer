...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        if ([predicate evaluateWithObject:initialStr])
        {
            ...
            //首字母大写
            chineseString.pinYin = [chineseString.string lowercaseString] ;
        }else{
            if(![chineseString.string isEqualToString:@""]){
                NSMutableString *pinyin = [chineseString.string mutableCopy];
                CFStringTransform((__bridge CFMutableStringRef)pinyin, NULL, kCFStringTransformMandarinLatin, NO);
                CFStringTransform((__bridge CFMutableStringRef)pinyin, NULL, kCFStringTransformStripDiacritics, NO);
                chineseString.pinYin=pinyin;
            }else{
...
//
//+(NSMutableArray*)ReturnselectArrar:(NSArray*)stringArr
//{
//    //获取字符串中文字的拼音与字符串共同存放
//    NSMutableArray *chineseStringsArray=[NSMutableArray array];
//    for(int i=0;i<[stringArr count];i++)
