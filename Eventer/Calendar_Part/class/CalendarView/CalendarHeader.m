...
@interface CalendarHeader ()<UICollectionViewDataSource,UICollectionViewDelegate>
...
- (void)initialize
{
    _dateFormat               = @"yyyy"@"年""MM""月";
    _dateFormatter            = [[NSDateFormatter alloc] init];
    _dateFormatter.dateFormat = _dateFormat;
    _minDissolveAlpha         = 0; // calendarheader 层叠率
...
- (void)layoutSubviews
{
    ...
    _collectionViewFlowLayout.itemSize = CGSizeMake(self._width * 0.5,
                                                    self._height);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDate *date = [_minimumDate _dateByAddingMonths:indexPath.item];
...
- (void)setScrollDirection:(UICollectionViewScrollDirection)scrollDirection
{
    if (_scrollDirection != scrollDirection) {
        ...
        if (scrollDirection == UICollectionViewScrollDirectionVertical) {
            _collectionViewFlowLayout.sectionInset = UIEdgeInsetsMake(0, self._width*0.25, 0, self._width*0.25);
        } else {
