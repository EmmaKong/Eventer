...
//从网络获取数据，此处先从数据库获取
    ...
    NSDictionary*results=[[databaseService shareddatabaseService]get:@"ALL" FromTable:@"dbCourse" WithCondition:nil];
    _coursesArray =[[NSMutableArray alloc]init];
    NSArray*array=[NSArray array];
    array=[results allValues];
    for (NSDictionary * obj in array) {
        WeekCourse*weekcourse=[[WeekCourse alloc]initWithPropertiesDictionary:obj];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.searchDisplayController.searchResultsTableView)
    {
        ...
        NSString *teachertext = [NSString stringWithFormat:@"%@",weekcourse.Teacher];
        NSString *classroomtext = [NSString stringWithFormat:@"%@",weekcourse.Place];
        NSString *seweektext = [NSString stringWithFormat:@"%@-%@,%@无课",weekcourse.StartWeek,weekcourse.EndWeek,weekcourse.NoClassWeek];
        NSString *daytext = [NSString stringWithFormat:@"%@",weekcourse.weekDay];
        NSString *lessonsnumtext = [NSString stringWithFormat:@"%@－%d节",weekcourse.lesson,weekcourse.lesson.intValue +weekcourse.lessonNum.intValue];
        ...
        NSString *seweektext = [NSString stringWithFormat:@"%@-%@,%@无课",weekcourse.StartWeek,weekcourse.EndWeek,weekcourse.NoClassWeek];
        NSString *daytext = [NSString stringWithFormat:@"%@",weekcourse.weekDay];
        NSString *lessonsnumtext = [NSString stringWithFormat:@"%@－%d节",weekcourse.lesson,weekcourse.lesson.intValue +weekcourse.lessonNum.intValue-1];
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    if (courseSearchBar.text.length>0&&![ChineseInclude isIncludeChineseInString:courseSearchBar.text]) {
        ...
        for (WeekCourse *course in _coursesArray) {
            NSString*tempStr=course.CourseName;
            NSRange titleResult=[tempStr rangeOfString:courseSearchBar.text options:NSCaseInsensitiveSearch];
            if (titleResult.length>0) {
                [searchResults addObject:course];
            }
        }
    }
}
