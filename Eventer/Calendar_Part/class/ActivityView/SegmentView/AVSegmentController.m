...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.segmentIndex=@"0";
    [[NSNotificationCenter defaultCenter]postNotificationName:@"segmentControlChangeState" object:self.segmentIndex];
    [self _baseConfigs];
    [self _baseLayout];
}
...
-(void)segmentControlDidChangedValue:(UISegmentedControl *)sender
{
    ...
    self.segmentIndex=[NSString stringWithFormat:@"%lu",(unsigned long)index];
    [[NSNotificationCenter defaultCenter]postNotificationName:@"segmentControlChangeState" object:self.segmentIndex];
